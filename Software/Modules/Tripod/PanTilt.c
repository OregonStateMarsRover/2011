...
void PanTiltHandlePacket(CommPacket * pkt) {
	...
	if (pkt->data[0]==SYS_PANTILT) { // make sure this is a packet that we're supposed to handle
		...
		if (pkt->data[1]==PANTILT_MOVEALL)
		{
			panPosition = ((pkt->data[3]<<8) & 0xFF00) | (pkt->data[4] & 0xFF);
			tiltPosition = ((pkt->data[5]<<8) & 0xFF00) | (pkt->data[6] & 0xFF);
		...
		if (pkt->data[1]==PANTILT_COMPLIANCE)
		{
